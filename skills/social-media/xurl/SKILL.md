...
### Read a Tweet (Primary Method — No Auth Required)

`api.fxtwitter.com` returns full tweet JSON including text, raw_text, created_at, engagement metrics (likes, retweets, views, bookmarks), and author data. Works for any public tweet with no authentication.
...
- `tweet.raw_text.text` — the actual tweet text content (URLs appear as `https://t.co/...`)
- `tweet.created_at` — full timestamp string
- `tweet.likes / retweets / views / bookmarks` — engagement metrics
- `tweet.lang` — `null` for link-only tweets, `"en"` for English text, `"zxx"` for non-text (article cards)

Use this for all tweet reading when `xurl read` is unavailable or returns 401. It is the most reliable method for public tweets.

### Read a Tweet via xurl
...
## Reading Tweets — Method Priority

> See `references/tweet-reading-via-fxtwitter.md` for detailed code examples and field documentation.
